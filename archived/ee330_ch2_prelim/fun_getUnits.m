function [unitOut] = fun_getUnits(type)
%{
 ...
 unitOut will be a (3 x 3) with each component in terms of original variable
    ...
    ie, you have a vector field in terms of CYN, say Avec = [ cr^2 , sin(cf) * cos(cf), 3 * cr ]
            you want to this field in terms of REC

        conv = fun_getUnits('CR')   % conv takes unitOut, is a (3x3) in terms of (cr, cf, cz)
...
%}   
    ...
    %{
    sym rx;                % rectangular params  rx, ry, rz
    %assume(rx, 'real');
    sym ry;
    %assume(ry,'real');
    sym rz;
    ...
    sym cz;
    %assume(cz, 'real');
    sym sr;                % spherical params  sr, st, sf
    %assume(sr, 'real');
    sym st;
    %assume(st,'real');
    sym sf;            
    %assume(sf, 'real');   % won't let me keep using syms sf   sym just as good ?
    %}                    
    if type == double('RC')
        %{ 
                                                            rec to cyn     RC
