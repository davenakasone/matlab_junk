...
if select == 3
    ...
    p_gauge = 65e3; % Pa, the gaugue pressure ball is pumped to
...
if select == 6
    ...
    rho_air = 1.23; % kg/m^3
    ballon_rho = 0.32; % kg/m^3
    ballon_volume = 0.075; % m^3
...
if select == 8
    ...
    iron_mass_air = 397.5; % g, mass in air
    iron_mass_sub = 347.5; % g, mass in unknown liquid
    iron_rho = 7.8; % g/cm^3
...
if select == 10
    Q = (215/60)/1000; % L/min --> L/s --> m^3/s 
    l = 49; % m
    d = 8.2/100; % m
    p_pump = 8.2e6; % N/m^2  "Pa"
    ...
    fprintf("b)  eta visc:  %0.3f  Ns/m^2    ???good!!!\n", viscosity);
...
if select == 12
    ...
    v = 2.2; % m/s   water moves in hose
    d = 1.35/100; % m   internal diameter of hose
    A = pi*(d/2)^2; % m^2
    Q_m3_p_s = A*v;
    liters_in_m3 = 1000;
...
if select == 13
    ...
    P_out = 680e6; % MW  power output from dam
...
if select == 14
    ...
    v_bot = 68; % m/s at take off    ground speed relative to bottom of wing
    rho_air = 1.29; % kg/m^3
...
if select == 15
  ...
  fprintf("c) pressure at nozzel:  %0.3f Pa  WTF??\n", p_noz);
